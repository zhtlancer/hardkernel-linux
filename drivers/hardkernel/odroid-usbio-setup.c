...
{
	if (kstrtoul(line, 10, &Orientation) != 0)
		Orientation = 0;
...
static int __init parse_touch(char *str, struct button *bt)
{
	...
	item = strsep(&str, ":");
	if (item != NULL) {
		if (kstrtoul(item, 10, &bt->x) != 0)
			return 0;
	}

	item = strsep(&str, ":");
	if (item != NULL) {
		if (kstrtoul(item, 10, &bt->y) != 0)
			return 0;
		return 1;
	}
	return 0;
...
/* parse bootargs */
	while (line != NULL) {
		...
		if (kstrtoul(item, 10, &key_num) != 0)
			return 0;

		if (key_num >= NUM_OF_KEYS)
			return 0;
		...
		switch (item[0]) {
		case 'K':
			BUTTON[key_num].type = BUTTON_TYPE_KEY;
			BUTTON[key_num].use =
				parse_key(str, &BUTTON[key_num]);
			break;

		case 'M':
			BUTTON[key_num].type = BUTTON_TYPE_MOUSE;
			BUTTON[key_num].use =
				parse_mouse(str, &BUTTON[key_num]);
			break;

		case 'T':
			BUTTON[key_num].type = BUTTON_TYPE_TOUCH;
			BUTTON[key_num].use =
				parse_touch(str, &BUTTON[key_num]);
			break;

		default:
			BUTTON[key_num].use = false;
		}
	}
	return 0;
}
		...
		const struct usb_device_id *id)
{
	...
	endpoint = usbio_sensor_get_input_endpoint(intf->cur_altsetting);
	if (!endpoint)
		return	-ENXIO;

	usbio = devm_kzalloc(&intf->dev, sizeof(struct usbio), GFP_KERNEL);
	if (!usbio)
		return	-ENOMEM;

	usbio->sensor = devm_kzalloc(&intf->dev, sizeof(struct sensor),
			GFP_KERNEL);
	if (!usbio->sensor)
		return	-ENOMEM;

	usbio->keypad = devm_kzalloc(&intf->dev, sizeof(struct keypad),
			GFP_KERNEL);
	if (!usbio->keypad)
		return	-ENOMEM;

	usbio->mouse = devm_kzalloc(&intf->dev, sizeof(struct mouse),
			GFP_KERNEL);
	if (!usbio->mouse)
		return	-ENOMEM;

	usbio->sensor->input = input_allocate_device();
	if (!usbio->sensor->input)
		return	-ENOMEM;

	usbio->keypad->input = input_allocate_device();
	if (!usbio->keypad->input)
		goto	err_free_mem;

	usbio->mouse->input = input_allocate_device();
	...
	usbio->irq = usb_alloc_urb(0, GFP_KERNEL);
	if (!usbio->irq)	{
	...
	if (udev->product) {
		if (udev->manufacturer)
			strlcat(usbio->name, " ", sizeof(usbio->name));
	...
	err = usbio_sensor_init(usbio);
	if (err)
		goto	err_free_urb;

	err = usbio_keypad_init(usbio);
	if (err)
		goto	err_free_urb;

	err = usbio_mouse_init(usbio);
	if (err)
		goto	err_free_urb;
...
static void usbio_disconnect(struct usb_interface *intf)
{
	...
	dev_dbg(&intf->dev,
			"%s - ODROID USBIO Sensor Board is initialized, cleaning up\n",
			__func__);
